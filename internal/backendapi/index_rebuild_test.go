...
func (a *adminUserContext) deliver(t *testing.T, body string) {
	...
	filename, vsize, guid, err := a.box.Save("INBOX", io.NopCloser(bytes.NewBufferString(body)), uid, int64(len(body)), nil, nil, [16]byte{})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	...
	if err := mailboxbase.NameSaved(a.box, "INBOX", filename, meta); err != nil {
		t.Fatalf("name: %v", err)
	}
