...
func TestASyncDoesNotTakeTheRowOfARecordItHasNotSeen(t *testing.T) {
	...
	if err := mbox.RecordSaved(writerIdx, writer, wf.ID, "INBOX", saved, m); err != nil {
		t.Fatal(err)
	}
	written := m.UID
	...
	name, perr := mbox.MessagePath(writer, "INBOX", msgs[0])
	if perr != nil {
...
func TestAnImportedFileKeepsTheUIDTheListGivesIt(t *testing.T) {
	...
	if _, perr := mbox.MessagePath(box, "INBOX", msgs[0]); perr != nil {
