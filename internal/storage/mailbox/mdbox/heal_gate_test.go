...
func TestASecondSessionDoesNotRepeatAFailedHeal(t *testing.T) {
	...
	if _, err := first.HealCorruptFolder(mailboxbase.Open(first, idx), idx, f); err == nil {
	...
	_, err := second.HealCorruptFolder(mailboxbase.Open(second, idx), idx, f)
	...
	if _, err := second.HealCorruptFolder(mailboxbase.Open(second, idx), idx, f); errors.Is(err, ErrHealDeferred) {
...
func TestTwoFoldersOfOneUserDoNotHealAtOnce(t *testing.T) {
	...
	for _, folder := range []*mailbox.Folder{
		...
		go func(f *mailbox.Folder) {
			defer wg.Done()
			_, _ = u.HealCorruptFolder(mailboxbase.Open(u, idx), idx, f)
		}(folder)
	}
	wg.Wait()
...
func TestBothKeysAreAlwaysTakenFolderFirst(t *testing.T) {
	...
	idx := &failingIndex{err: errors.New("scan incomplete")}
	_, _ = u.HealCorruptFolder(mailboxbase.Open(u, idx), idx, &mailbox.Folder{ID: 7, Name: "INBOX", Fsckd: true})
