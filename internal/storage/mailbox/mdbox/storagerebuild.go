...
func (u *userMailbox) RebuildStorage(box mailbox.Box, idx mailbox.UserIndex, restoreOrphans bool) (mailbox.StorageRebuildStats, error) {
	...
	err = u.withMapLock(func() error {
		...
		// re-filed.
		if restoreOrphans {
			restored, rerr := u.restoreTaggedOrphans(box, idx, present, refCount)
			if rerr != nil {
				return rerr
			}
...
func (u *userMailbox) restoreTaggedOrphans(box mailbox.Box, idx mailbox.UserIndex, present map[string]*mailbox.ScanRecord, refCount map[uint32]int) (int, error) {
