...
func TestHealExpungesVanishedAndClearsMarker(t *testing.T) {
	...
	expunged, err := box.HealCorruptFolder(mailboxbase.Open(box, idx), idx, f)
	if err != nil {
		t.Fatalf("heal: %v", err)
	}
	if len(expunged) != 1 {
...
func TestHealAbortsOnVanishedFileMidScan(t *testing.T) {
	...
	f, _ := idx.OpenFolder("INBOX", 0)
	if _, err := box.HealCorruptFolder(mailboxbase.Open(box, idx), idx, f); err == nil {
...
func TestHealAbortsOnIncompleteScan(t *testing.T) {
	...
	f, _ := idx.OpenFolder("INBOX", 0)
	_, err = box.HealCorruptFolder(mailboxbase.Open(box, idx), idx, f)
	if err == nil {
