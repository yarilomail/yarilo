...
for type in $TYPES; do
 ...
 for run in $(seq 1 "$REPEATS"); do
  ...
  echo "$ARM $name $(awk -v logins="${logins:-0}" '
    ...
    ' "$OUT/backend-$ARM-$name-delta.txt")"
  # Where a listing's cost comes from, per login: the list, cur/, and why the
  # cached listing could not be used (#1875).
  echo "$ARM $name $(awk -v logins="${logins:-0}" '
      $1 ~ /^maildir_uidlist_read_total\{/ && $1 ~ /mode="whole"/ { whole += $2 }
      $1 ~ /^maildir_uidlist_read_total\{/ && $1 ~ /mode="tail"/ { tail += $2 }
      $1 ~ /^maildir_dir_read_total\{/ && $1 ~ /reason="current-name"/ { byName += $2 }
      $1 ~ /^maildir_dir_read_total\{/ && $1 ~ /reason="scan"/ { byScan += $2 }
      ...
      $1 ~ /^maildir_listing_retry_total\{/ && $1 ~ /at="open"/ { retryOpen += $2 }
      ...
      END { printf "listing: list_whole=%d list_tail=%d dir_name=%d dir_scan=%d dir_remove=%d miss_none=%d miss_stale=%d retry_open=%d retry_rename=%d stat_dir=%d stat_list=%d",
              whole, tail, byName, byScan, byRemove, missNone, missStale, retryOpen, retryRename, statDir, statList
