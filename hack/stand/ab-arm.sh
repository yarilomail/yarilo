...
for pair in "mdbox 1-20" "maildir 51-70" "sdbox 101-120"; do
 ...
 for run in $(seq 1 "$REPEATS"); do
  ...
  echo "$ARM $name $(awk -v logins="${logins:-0}" '
      ...
      END { printf "listing: list_whole=%d list_tail=%d dir_name=%d dir_scan=%d dir_remove=%d miss_none=%d miss_stale=%d",
              whole, tail, byName, byScan, byRemove, missNone, missStale
            if (logins + 0 > 0) printf " list_per_login=%.3f dir_per_login=%.3f",
              (whole + tail) / logins, (byName + byScan + byRemove) / logins }
    ' "$OUT/backend-$ARM-$name-delta.txt")"
